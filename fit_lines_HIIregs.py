...
#####        SNR4449 = np.loadtxt(path_data + 'spec_snr4449_mmt2008_z.flm')
 ...
 ########       cc_SNR4449 = np.zeros( ( nrows, ncols, nreg_cc ) )
        ...
        tbhdu = [0]*(8) # header, cols
        clmns = [0]*(7) # flux, eflux, vel, evel, linepars, elinepars, fluxint, cc_CasA, cc_SNR4449
...
#        clmns[7] = fits.ImageHDU(ew.T, name = 'ew')
#        clmns[7].header['CONTENT'] = 'EQUIVALENT WIDTH','A]'
#        clmns[7].header['PLANE01'] = 'Halpha 6562.77', 'A'
#        clmns[7].header['PLANE02'] = 'Hbeta 4861.33', 'A'


 #       clmns[8] = fits.ImageHDU(dew.T, name = 'dew')
  #      clmns[8].header['CONTENT'] = 'EQUIVALENT WIDTH ERRORS','A]'
   #     clmns[8].header['PLANE01'] = 'Halpha 6562.77', 'A'
   #     clmns[8].header['PLANE02'] = 'Hbeta 4861.33', 'A'
        ...
        hdulist.writeto(path_write + cube_name + '_%s.HII_regs.lines.fits' % (survey), overwrite = True)
...
def MakeGasCube(cube_name):
        ...
        hdr=fits.open('Final_Sample/'+cube_name+'/'+cube_name+'.resam.fits')[0].header

        outs = glob.glob('Final_Sample/'+cube_name+'/HII_regs/'+cube_name+'_*.C11*')

        inputs = glob.glob('Final_Sample/'+cube_name+'/HII_regs/'+cube_name+'_*.txt')
        seg_map = fits.open('resultados_'+gal+'_'+gal+'_map_seg.fits')[0].data
        ini_cubo=int(hdr['CRVAL3'])   
        cubo = np.zeros((int(hdr['NAXIS3']+1),1,int(np.max(seg_map))+1))
        errores = np.zeros((int(hdr['NAXIS3']+1),1,int(np.max(seg_map))+1))
        for bin_index in range(1, int(np.max(seg_map))+1):
            try:
                file_ = [s for s in outs if "_reg_"+str(bin_index)+"." in s][0]
                file_in = [s for s in inputs if "_reg_"+str(bin_index)+"." in s][0]
        ...
        hdu_flux=fits.PrimaryHDU(cubo.data,header=hdr)
        hdu_error=fits.ImageHDU(errores.data,header=hdr,name='errores')
        hdu_new=fits.HDUList([hdu_flux,hdu_error])
        hdu_new.writeto('HII_regs_'+cube_name+'.fits', overwrite=True)
...
if __name__ == '__main__':
    ...
    fit_emission_absorption_lines_w('HII_regs_'+args.Cubename,'MUSE','/home/raul.gonzalez/raul/STARLIGHTv04/',0,3)
    elapsed = (time.time() - start_time)
    print(str(timedelta(seconds=elapsed)))
