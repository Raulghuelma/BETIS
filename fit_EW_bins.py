...
#start_time = time.time()
        ...
        for bin_index in range(1, int(np.max(bin_map))):
            try:
                ...
                data_out = ascii.read(file_, data_start=235)['col3']
           ...
           # end_model=ascii.read(file_, data_start=235)['col1'][-1]
                ...
                error = ascii.read(file_in)['col3']
                for i,j in zip(x,y):
               
                    cubo.data[np.abs(ini_model-ini_cubo):np.abs(9100-int(ini_cubo)),int(i),int(j)]=(np.array(data_in)/np.array(data_out))[:9100-ini_model]
        ...
        hdu_new.writeto('stellar_'+cube_name+'_binned.fits', overwrite=True)
      #  hdu_new.writeto(cube_name+'_binned.fits', overwrite=True)
...
if __name__ == '__main__':
    ...
    fit_emission_absorption_lines_w('stellar_'+args.Cubename+'_binned','MUSE','/home/raul.gonzalez/raul/STARLIGHTv04/',0,3)
    elapsed = (time.time() - start_time)
    print(str(timedelta(seconds=elapsed)))
