...
def segmentate_cube(sn_min, gal):

    table = ascii.read('TABLE_MASTER_Final_Sample.tsv')

    signal = fits.open('/home/hidra2/rgonzalez/sn_maps/'+gal+'_S_SII_map.fits')[0].data
    
    noise = fits.open('/home/hidra2/rgonzalez/sn_maps/'+gal+'_N_SII_map.fits')[0].data
    
    z = float(table['z'][table['galaxy']==gal].value[0])
    a_v = float(table['a_v'][table['galaxy']==gal].value[0])
    ...
    path = '/home/hidra2/rgonzalez/Final_Sample/'+gal+'/'
    try:
        os.mkdir('/home/hidra2/rgonzalez/Final_Sample/'+gal+'/adabin_out_SII/')
    ...
    cube = fits.open(path+gal+'.fits')[1].data*1e-20
    cube_error = fits.open(path+gal+'.fits')[2].data*1e-20
    cube_hdr = fits.open(path+gal+'.fits')[1].header

    f = open('/home/hidra2/rgonzalez/Final_Sample/temp.in')
    ...
    for i in range(1, int(np.max(bin_map)+1)):
        ...
        if len(x_)>0:
            ...
            a_lambda = fitzpatrick99(wavelength, a_v, 3.1)
            ...
            resampled_flux = np.empty(len(resampled_wavelength))
            resampled_eflux = np.empty(len(resampled_wavelength))
            
            unreddened_flux = spec * (1 + z) * correction_factor
            unreddened_eflux = err * (1 + z) * correction_factor
            
            resampled_flux = np.interp(resampled_wavelength, 
                                                restframed_wavelength, 
                                                unreddened_flux)
            resampled_eflux = np.interp(resampled_wavelength, 
                                                restframed_wavelength,
                                                unreddened_eflux)

        
            if np.median(resampled_flux) == 0.0: 
                resampled_flux = resampled_flux*np.nan
            ...
            np.savetxt(path+'adabin_out_SII/'+gal+'_bin_'+(str(i))+'.txt', np.array([resampled_wavelength, resampled_flux, resampled_eflux, flag]).T, delimiter=' ', fmt="%s")
