 ...
 #       clmns[8] = fits.ImageHDU(dew.T, name = 'dew')
  #      clmns[8].header['CONTENT'] = 'EQUIVALENT WIDTH ERRORS','A]'
   ...
   #     clmns[8].header['PLANE02'] = 'Hbeta 4861.33', 'A'
        ...
        hdulist.writeto(path_write + cube_name + '_%s.binned.lines.fits' % (survey), overwrite = True)
...
def MakeGasCube(cube_name):
        ...
        for bin_index in range(1, int(np.max(bin_map))):
           ...
           # end_model=ascii.read(file_, data_start=235)['col1'][-1]
                ...
                for i,j in zip(x,y):    
                    cubo.data[np.abs(ini_cubo-ini_model):np.abs(9100-int(ini_cubo)),int(i),int(j)]=10**20*(np.array(data_in)-np.array(data_out))[:9100-ini_model]*ascii.read(file_,data_start=16,data_end=17)['col1']

                    errores.data[np.abs(ini_cubo-ini_model):np.abs(9100-int(ini_cubo)),int(i),int(j)] = 10**20*np.array(error[np.abs(ini_cubo-ini_model):np.abs(9100-int(ini_cubo))])
        ...
        hdu_new.writeto('binned_'+cube_name+'.fits', overwrite=True)
...
if __name__ == '__main__':
    ...
    fit_emission_absorption_lines_w('binned_'+args.Cubename,'MUSE','/home/raul.gonzalez/raul/STARLIGHTv04/',0,3)
    elapsed = (time.time() - start_time)
    print(str(timedelta(seconds=elapsed)))
