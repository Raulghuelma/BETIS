 ...
 ########       cc_SNR4449 = np.zeros( ( nrows, ncols, nreg_cc ) )
        ...
        for i in rows:
            ...
            #    print(i, j)
                ...
                if (np.sum(sp) > 0.) or (np.sum(sp) < 0.): # Remove bad spectra
...
def MakeGasCube(cube_name):
        ...
        for bin_index in range(1, int(np.max(bin_map))):
            try:
                ...
                cubo[:, 0, bin_index] = 10**20 * (np.array(data_in) - np.array(data_out)) * ascii.read(file_, data_start = 16, data_end = 17)['col1']
        ...
        hdu_new.writeto('ALT_binned_nebular_specs_' + cube_name + '.fits', overwrite=True)
...
if __name__ == '__main__':
    ...
    fit_emission_absorption_lines_w('ALT_binned_nebular_specs_'+args.Cubename,'MUSE','',0,3)
    elapsed = (time.time() - start_time)
    print(str(timedelta(seconds=elapsed)))
