...
#start_time = time.time()
        ...
        list_= glob.glob('Final_Sample/'+cube_name+'/'+cube_name+'_*.BN')
        if hdr['CRVAL3']<=4700.0:
            start=int(4700.0 - hdr['CRVAL3'])
        else:
            start=0
        ...
        for file_ in list_:
            ...
            try:
                ...
                data_ = ascii.read(file_, data_start=235)['col3']
      
      #          print(file_)
       
                data_in = ascii.read(file_, data_start=235)['col2']

                a=file_.replace('Final_Sample/'+cube_name+'/'+cube_name+'_', '').replace('.C11.gm.CCM.BN','')
         
                x,y=a.split('_')
      
                end=int(ascii.read(file_, data_start=235)['col1'][-1])
                if end <= (hdr['CRVAL3']+hdr['NAXIS3']):
                    end_cube=int(hdr['CRVAL3']+hdr['NAXIS3']-end)
                    end_file=end
                else:
                    end_file=int(end-hdr['CRVAL3']-hdr['NAXIS3'])
                    end_cube=0
     
                ini_cubo=int(hdr['CRVAL3'])
                ini_model=int(ascii.read(file_, data_start=235)['col1'][0])
 
                cubo.data[np.abs(ini_cubo-ini_model):np.abs(9200-int(ini_cubo)+1),int(x),int(y)]=(data_in-data_)*10**20*ascii.read(file_, data_start=16,data_end=17)['col1']
                #print(int(ascii.read(file_, data_start=235)['col1'][-1]))
            except KeyError: 'col3'
     
        hdr['CDELT3']=1.0
        hdr['CD3_3']=1.0
        hdr['NAXIS3']=len(data_)
        ...
        hdu_new.writeto(cube_name+'.fits', overwrite=True)
...
if __name__ == '__main__':
    ...
    fit_emission_absorption_lines_w(args.Cubename,'MUSE','/home/raul.gonzalez/raul/STARLIGHTv04/',0,3)
    elapsed = (time.time() - start_time)
    print(str(timedelta(seconds=elapsed)))
