def multipeak(p, x, y, err, fjac=None, fit=False, npeak=0):
    ...
    from lib.math_functions import gaussian, polynomial
...
def multipeak_fit(x, y, initpars, npeak, parinfo, fit=False, \
    ...
    if plot: # Plot data and final model
        ...
        for nmp in range(npeak):
            plt.plot(x, gaussian(x, p.params[3*nmp:3*nmp+3]) + polynomial(x, p.params[3*npeak:]),\
                color=color_array[nmp])
